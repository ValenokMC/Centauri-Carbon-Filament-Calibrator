...
- Owner: **ValenokMC**
- Name: **Centauri-Carbon-Filament-Calibrator**
- Visibility: **Public**
- **Do not** let GitHub add a README, a `.gitignore` or a licence — they are
  already here and would collide.

```bash
gh repo create ValenokMC/Centauri-Carbon-Filament-Calibrator --public --source . --remote origin
```

or add the remote by hand:

```bash
git remote add origin https://github.com/ValenokMC/Centauri-Carbon-Filament-Calibrator.git
```
...
> Filament calibration for the Elegoo Centauri Carbon and OrcaSlicer. Measure a spool, get a preset.
...
| **Show and tell** | Open discussion | Calibration results, spools people have measured. |
...
python tools/check_public_safety.py dist/Centauri-Calibrator-v1.0.0-windows.zip
```
...
(`Centauri-Calibrator-v1.0.0-windows.zip` and `SHA256SUMS.txt`), then publish.

**Before publishing, download your own archive, unpack it somewhere clean, and
run `Setup.cmd` and then `Doctor.cmd`.** It is the only way to find out that
something a user needs was left out of the allow-list — the shipped shrinkage
plates in particular.

## 13. Pin it

**Your profile → Customize your pins.** Pin both this repository and the
Telegram bot, so the two read as one line of tools.
